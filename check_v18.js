...
function updateBoardLayout(){
  ...
  if(isMobileBoard()){
    // v17 mobile: keep the original isometric board geometry intact.
    // Make it larger by cropping the SVG viewBox to the actual board bounds,
    // not by changing CELL_W / CELL_H / SKEW_X, which breaks the projection.
    ...
    if(svg){
      // Tight crop around all four layers and pieces. This removes the false
      // blank area below the board while preserving the correct 3D shape.
      svg.setAttribute('viewBox','185 28 555 815');
